...
# def reference_selected_version(pm, asset_type, asset, subtype, version_filename, namespace=None,
#                                group_locator=True, merge_namespaces=False):
#     asset_dir = pm.get_asset_dir(asset_type, asset)
#     workshop_dir = os.path.join(asset_dir, 'components', subtype, 'workshop')
#     file_path = os.path.join(workshop_dir, version_filename)
#     if not os.path.exists(file_path):
#         raise FileNotFoundError(u"版本文件不存在: {}".format(file_path))
#     return reference_file(file_path, namespace=namespace, group_locator=group_locator,merge_namespaces=merge_namespaces)