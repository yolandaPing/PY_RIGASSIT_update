...
class Shape():
    def __init__(self, options):
        ...
        self.submenu_data = options.get('submenu_data')  # 新增
        self.has_submenu = self.submenu_data is not None  # 新增
    ...
    def draw(self, painter):
        ...
        if self.has_submenu:
            self._draw_submenu_indicator(painter)
    ...
    def _draw_submenu_indicator(self, painter):
        # 在按钮右侧绘制小箭头
        arrow_rect = QtCore.QRectF(
            self.rect.right() - 15,
            self.rect.center().y() - 5,
            10, 10
        )
        color = QtGui.QColor(255, 200, 100) if self.hovered else QtGui.QColor(200, 200, 200)
        painter.setPen(QtGui.QPen(color))
        painter.setBrush(QtGui.QBrush(color))
        points = [
            QtCore.QPointF(arrow_rect.left(), arrow_rect.top()),
            QtCore.QPointF(arrow_rect.right(), arrow_rect.center().y()),
            QtCore.QPointF(arrow_rect.left(), arrow_rect.bottom())
        ]
