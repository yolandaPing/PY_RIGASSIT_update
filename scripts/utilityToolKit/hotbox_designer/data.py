...
# 新增
def save_nested_hotbox(hotbox_data):
    """递归保存子菜单数据"""
    # 确保不会循环引用（可选检查）
    for shape in hotbox_data.get('shapes', []):
        if shape.get('submenu_data'):
            shape['submenu_data'] = save_nested_hotbox(shape['submenu_data'])
    return hotbox_data

def load_nested_hotbox(hotbox_data):
    """递归加载子菜单数据"""
    for shape in hotbox_data.get('shapes', []):
        if shape.get('submenu_data'):
            shape['submenu_data'] = load_nested_hotbox(shape['submenu_data'])
    return hotbox_data