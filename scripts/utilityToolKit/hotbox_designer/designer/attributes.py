...
LEFT_CELL_WIDTH = 80
SHAPE_TYPES = 'square', 'round', "triangle", "diamond", "hexagon", "star", "line"
...
class AttributeEditor(QtWidgets.QWidget):
    ...
    submenuEditRequested = QtCore.Signal(object)  
    submenuClearRequested = QtCore.Signal(object)  
    ...
    def __init__(self, application, parent=None):
        ...
        self.submenu = SubmenuSettings()
        self.submenu.editSubmenuRequested.connect(self._on_submenu_edit)
        self.submenu.clearSubmenuRequested.connect(self._on_submenu_clear)
        self.submenu_toggler = WidgetToggler('Nested Submenu', self.submenu)

        self.layout.addWidget(self.submenu_toggler)
        self.layout.addWidget(self.submenu)

        # 在 set_options 方法中添加：
        # self.submenu.set_options(options)
    ...
    def set_options(self, options):
        ...
        self._current_shape_options = options  # 保存以备子菜单编辑使用
        self.submenu.set_options(options)
    ...
    def _on_submenu_edit(self):
        if hasattr(self, '_current_shape_options') and self._current_shape_options:
            self.submenuEditRequested.emit(self._current_shape_options[0])

    def _on_submenu_clear(self):
        """处理删除子菜单请求"""
        if hasattr(self, '_current_shape_options') and self._current_shape_options:
            self.submenuClearRequested.emit(self._current_shape_options[0])
...
class SubmenuSettings(QtWidgets.QWidget):
    editSubmenuRequested = QtCore.Signal()
    clearSubmenuRequested = QtCore.Signal()

    def __init__(self, parent=None):
        super(SubmenuSettings, self).__init__(parent)
        # self._clear_btn_color = ("color: black;", "color: red;")
        self.edit_btn = QtWidgets.QPushButton("Add/Edit nested Submenu")
        # self.edit_btn.setStyleSheet("color: green;")
        self.edit_btn.clicked.connect(self.editSubmenuRequested.emit)
        self.clear_btn = QtWidgets.QPushButton("Delete nested Submenu")
        # self.clear_btn.setStyleSheet(self._clear_btn_color[0])
        self.clear_btn.clicked.connect(self.clearSubmenuRequested.emit)
        self.info_label = QtWidgets.QLabel("No submenu attached")
        self.info_label.setStyleSheet("font: bold ; color: red;")
        ...
        self._current_has_submenu = False

    def set_options(self, options):
        # options 是当前选中的形状选项列表
        if len(options) == 1:
            has_submenu = options[0].get('submenu_data') is not None
            self._current_has_submenu = has_submenu
            self.info_label.setText("> Has submenu: Yes" if has_submenu else "> No submenu")
            self.info_label.setStyleSheet("font: bold ; color: yellow;" if has_submenu else "font: bold ; color: red;")
            self.edit_btn.setEnabled(True)
            self.clear_btn.setEnabled(has_submenu)
            # self.clear_btn.setStyleSheet(self._clear_btn_color[has_submenu])
        else:
            self.info_label.setText("Multiple shapes selected")
            self.edit_btn.setEnabled(False)
            self.clear_btn.setEnabled(False)
            # self.clear_btn.setStyleSheet(self._clear_btn_color[0])

    def _clear_submenu(self):
        self.editSubmenuRequested.emit()

    def _on_edit(self):
        # print("Edit Submenu button clicked")
        if self._current_shape_options:
            self.editSubmenuRequested.emit(self._current_shape_options)
        else:print("No shape selected")